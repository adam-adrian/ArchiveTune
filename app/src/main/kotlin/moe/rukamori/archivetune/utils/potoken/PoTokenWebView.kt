 ...
 * Original: com.metrolist.music.utils.potoken.PoTokenWebView
 */
...
class PoTokenWebView private constructor(
    ...
    fun onObtainPoTokenError(identifier: String, error: String) {
        Timber.tag(TAG).e("onObtainPoTokenError: identifier=$identifier error=$error")
    ...
    fun onObtainPoTokenResult(identifier: String, poToken: String) {
        // poToken comes as comma-separated uint8 array, convert to base64
        try {
            val decoded = u8ToBase64(poToken)
            Timber.tag(TAG).d("onObtainPoTokenResult: identifier=$identifier poToken=${decoded.take(50)}...")
            popPoTokenContinuation(identifier)?.resume(decoded)
        } catch (e: Exception) {
            Timber.tag(TAG).e(e, "Failed to decode poToken")
            popPoTokenContinuation(identifier)?.resumeWithException(buildExceptionForJsError("Failed to decode poToken: ${e.message}"))
        }
    }
    ...
    companion object {
        ...
        private val httpClient = OkHttpClient.Builder()
            .proxy(moe.rukamori.archivetune.innertube.YouTube.proxy)
            .build()
