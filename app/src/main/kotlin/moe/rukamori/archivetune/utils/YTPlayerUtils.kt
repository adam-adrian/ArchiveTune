...
object YTPlayerUtils {
    ...
    private suspend fun playerResponseForPlaybackOnce(
        ...
        if (metadataClient.useWebPoTokens && sessionId != null) {
            try {
                val poTokenResult = WebViewPoTokenGenerator.getWebClientPoToken(videoId, sessionId)
        ...
        if (metadataFailure != null && canUseLoggedInPlayback && metadataFailure.isInvalidPlaybackLoginContextFailure()) {
            ...
            if (metadataClient.useWebPoTokens && authState.visitorData != null) {
                try {
                    val poTokenResult = WebViewPoTokenGenerator.getWebClientPoToken(videoId, authState.visitorData!!)
                    metadataPoToken = poTokenResult?.playerRequestPoToken
                } catch (e: Exception) {
    ...
    suspend fun playerResponseForMetadata(
        ...
        Timber.tag(logTag).i("Fetching metadata player response for videoId: $videoId using MAIN_CLIENT: ${MAIN_CLIENT.clientName}")
        
        val signatureTimestamp = getSignatureTimestampOrNull(videoId)
        val isLoggedIn = authState.hasPlaybackLoginContext
        // Always use visitorData for PoToken session (PR #3950 fix)
        val sessionId = authState.visitorData
